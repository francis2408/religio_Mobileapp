...
class HolyOrderScreen extends StatefulWidget {
  const HolyOrderScreen({Key? key}) : super(key: key);

  @override
  State<HolyOrderScreen> createState() => _HolyOrderScreenState();
}

class _HolyOrderScreenState extends State<HolyOrderScreen> {
  ...
  deleteData() async {
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.maybePop(context, MaterialPageRoute(builder: (context) => HolyOrderScreen()),).then((res) => setState(() {
        _isLoading = true;
        getHolyOrderData();
      }));
