...
class ProfileHealthScreen extends StatefulWidget {
  const ProfileHealthScreen({Key? key}) : super(key: key);

  @override
  State<ProfileHealthScreen> createState() => _ProfileHealthScreenState();
}

class _ProfileHealthScreenState extends State<ProfileHealthScreen> {
  ...
  deleteData() async {
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.maybePop(context, MaterialPageRoute(builder: (context) => ProfileHealthScreen()),).then((res) => setState(() {
        _isLoading = true;
        getHealthData();
      }));
