...
class ProfileFormationScreen extends StatefulWidget {
  const ProfileFormationScreen({Key? key}) : super(key: key);

  @override
  State<ProfileFormationScreen> createState() => _ProfileFormationScreenState();
}
...
class _ProfileFormationScreenState extends State<ProfileFormationScreen> {
  ...
  deleteData() async {
    ...
    if (response.statusCode == 200) {
      ...
      Navigator.maybePop(context, MaterialPageRoute(builder: (context) => ProfileFormationScreen()),).then((res) => setState(() {
        _isLoading = true;
        getFormationData();
      }));
