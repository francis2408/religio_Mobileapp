...
class EditHealthScreen extends StatefulWidget {
  const EditHealthScreen({Key? key}) : super(key: key);
...
class _EditHealthScreenState extends State<EditHealthScreen> {
  ...
  getHealthData() async {
    ...
    Map data = {
      "params": {
        "filter": "[['member_id','=',${userProfile == "Profile" ? userMember : memberId}]]",
        "query": "{id,member_id,start_date,end_date,disease_disorder_id,particulars,referred_physician,insurance_details,disease_description}"
      }
    };
    ...
    if (response.statusCode == 200) {
      ...
      for(int i = 0; i < healthData.length; i++) {
        if(healthData[i]['disease_disorder_id']['id'] != "" && healthData[i]['disease_disorder_id']['name'] != "") {
          diseaseID = healthData[i]['disease_disorder_id']['id'];
          diseaseName = healthData[i]['disease_disorder_id']['name'];
        } else {
          diseaseName = '';
        }
        startDateController.text = healthData[i]['start_date'];
        startDate = healthData[i]['start_date'];
        endDateController.text = healthData[i]['end_date'];
        endDate = healthData[i]['end_date'];
        concernController.text = healthData[i]['particulars'];
        physicianController.text = healthData[i]['referred_physician'];
        descriptionController.text = healthData[i]['disease_description'];
      }
    }
    else {
  ...
  update(String diseaseName) async {
    if(diseaseName != null && diseaseName != '') {
      ...
      String url = '$baseUrl/edit/member.health/$healthId';
      Map data = {
        "params":{
                  ...
                  children: [
                    ...
                    Container(
                      ...
                      child: DropDownTextField(
                        ...
                        textFieldDecoration: InputDecoration(
                          hintText: diseaseName != '' ? diseaseName : "Select disease type",
                    ...
                    Container(
                      ...
                      child: TextFormField(
                        ...
                        decoration: InputDecoration(
                          hintText: "Your referred physician",
                          border: OutlineInputBorder(
                              borderRadius: BorderRadius.circular(10)
                          ),
                          hintStyle: GoogleFonts.breeSerif(
