  ...
  save(level, study, mode) async {
    if(level != null && level != '' &&
      ...
      if(isFile != true) {
        ...
        String url = '$baseUrl/create/member.education';
        Map data = {
          "params": {
            "data":{"study_level_id": levelID,"member_id": userProfile == "Profile" ? userMember : memberId,"program_id": studyID,"particulars": particular,"year_of_passing": years,"institution": institute,"mode": mode,"result": result,"status": status,"attachment": _attacFile != '' ? baseFile : _attacFile}
          }
        };
        var body = jsonEncode(data);
        var response = await http.post(Uri.parse(url),
                ...
                child: TextButton(
                    onPressed: () {
                      if(level.isNotEmpty && study.isNotEmpty && mode.isNotEmpty) {
                        ...
                        // educationProviderModel.add(context);
