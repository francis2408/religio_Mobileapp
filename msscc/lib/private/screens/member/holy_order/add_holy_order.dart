...
class AddHolyOrderScreen extends StatefulWidget {
  const AddHolyOrderScreen({Key? key}) : super(key: key);

  @override
  State<AddHolyOrderScreen> createState() => _AddHolyOrderScreenState();
}

class _AddHolyOrderScreenState extends State<AddHolyOrderScreen> {
  ...
  save(String order, date, status) async {
    if(order != null && order != '' &&
        date != null && date != '' &&
        status != null && status != '') {
      String place = placeController.text.toString();
      String minister = ministerController.text.toString();
      var request = http.MultipartRequest('POST',  Uri.parse('$baseUrl/create/res.holyorder'));
      ...
      if(response.statusCode == 200) {
        ...
        setState(() {
          ...
          AnimatedSnackBar.show(
              context,
              'Holy order data created successfully.',
              Colors.green
          );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: screenBackgroundColor,
        appBar: AppBar(
          title: const Text('Add Holy Order'),
          centerTitle: true,
