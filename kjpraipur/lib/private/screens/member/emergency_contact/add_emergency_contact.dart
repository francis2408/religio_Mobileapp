...
class _AddEmergencyContactScreenState extends State<AddEmergencyContactScreen> {
  ...
  save(String name) async {
    if(name.isNotEmpty && name != '') {
      ...
      String relation = '';
      relationShipID != '' ? relationShipID : relation;

      var request = http.MultipartRequest('POST',  Uri.parse('$baseUrl/create/emergency.contact.info'));
      userMember == 'Member' ? request.fields.addAll({
        'values': "{'member_id': $id,'emer_name': '$name','emer_phone': '$mobile','emer_relationship_id': '$relationShipID','emer_street': '$street','emer_street2': '$street2','emer_place': '$place','emer_city': '$city','emer_district_id': '$districtID','emer_state_id': '$stateID','emer_country_id': '$countryID','emer_zip': '$zip'}"
      }) : request.fields.addAll({
        'values': "{'member_id': $memberId,'emer_name': '$name','emer_phone': '$mobile','emer_relationship_id': '$relationShipID','emer_street': '$street','emer_street2': '$street2','emer_place': '$place','emer_city': '$city','emer_district_id': '$districtID','emer_state_id': '$stateID','emer_country_id': '$countryID','emer_zip': '$zip'}"
      });
      request.headers.addAll(headers);
      http.StreamedResponse response = await request.send();
      if(response.statusCode == 200) {
        ...
        setState(() {
          ...
          AnimatedSnackBar.show(
              context,
              'Emergency data created successfully.',
              Colors.green
          );
          Navigator.pop(context);
