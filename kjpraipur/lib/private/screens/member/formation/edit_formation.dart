...
class _EditFormationScreenState extends State<EditFormationScreen> {
  ...
  getHouseData() async {
    var request = http.Request('GET', Uri.parse("$baseUrl/search_read/res.community?fields=['name']"));
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    if (response.statusCode == 200) {
      List data = json.decode(await response.stream.bytesToString())['data'];
  ...
  update(String stage, startYear, state) async {
    if(stage != null && stage != '' && startYear != null && startYear != '') {
      ...
      userMember == 'Member' ? request.fields.addAll({
        'values': "{'formation_stage_id': $stageID,'member_id': $id,'house_id': [$houseID],'start_year': '$startYear','end_year': '$endYear','study_info': '$studyInfo','state': '$state'}"
      }) : request.fields.addAll({
        'values': "{'formation_stage_id': $stageID,'member_id': $memberId,'house_id': [$houseID],'start_year': '$startYear','end_year': '$endYear','study_info': '$studyInfo','state': '$state'}"
      });
      request.headers.addAll(headers);
      http.StreamedResponse response = await request.send();
      if(response.statusCode == 200) {
