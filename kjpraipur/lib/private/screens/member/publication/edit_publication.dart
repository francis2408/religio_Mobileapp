...
class EditPublicationScreen extends StatefulWidget {
  const EditPublicationScreen({Key? key}) : super(key: key);
...
class _EditPublicationScreenState extends State<EditPublicationScreen> {
  ...
  getPublicationData() async {
    var request = http.Request('GET', Uri.parse("$baseUrl/search_read/res.publication/$publicationId"));
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    ...
    if (response.statusCode == 200) {
      ...
      for(int i = 0; i < publicData.length; i++) {
        titleController.text = publicData[i]['title'];
        DateTime nextDate = DateTime.parse(publicData[i]['publication_date']);
        String nextDateString = DateFormat('dd-MM-yyyy').format(nextDate);
        dateController.text = nextDateString;
        date = publicData[i]['publication_date'];
        if(publicData[i]['publication_type_id'] != '' && publicData[i]['publication_type_id'] != null && publicData[i]['publication_type_id'] != []) {
          publicID = publicData[i]['publication_type_id'][0].toString();
          publication = publicData[i]['publication_type_id'][1];
        }
        publisherController.text = publicData[i]['publisher'];
        royaltyController.text = publicData[i]['royalty'];
      }
    } else {
      var message = json.decode(await response.stream.bytesToString())['message'];
      setState(() {
  ...
  update(String title, dates) async {
    if(title.isNotEmpty && title != '' && dates != null && dates != '') {
      ...
      var request = http.MultipartRequest('PUT',  Uri.parse('$baseUrl/write/res.publication?ids=[$publicationId]'));
      userMember == 'Member' ? request.fields.addAll({
        'values': "{'member_id': $id,'title': '$title','publication_date': '$date','publication_type_id': [$publicID],'publisher': '$publisher','royalty': '$royalty'}"
      }) : request.fields.addAll({
        'values': "{'member_id': $memberId,'title': '$title','publication_date': '$date','publication_type_id': [$publicID],'publisher': '$publisher','royalty': '$royalty'}"
      });
      request.headers.addAll(headers);
      http.StreamedResponse response = await request.send();
      if(response.statusCode == 200) {
        ...
        setState(() {
          ...
          AnimatedSnackBar.show(
              context,
              'Publication data updated successfully.',
              Colors.green
          );
          Navigator.pop(context);
                    ...
                    Container(
                      ...
                      child: DropDownTextField(
                        ...
                        textFieldDecoration: InputDecoration(
                          ...
                          hintText: publication != '' ? publication : "Select publication type",
                          hintStyle: GoogleFonts.breeSerif(
                            // fontWeight: FontWeight.bold,
                            color: Colors.black87,
                          ),
                      ...
                      onPressed: () {
                        if(titleController.text.toString().isNotEmpty && dateController.text.isNotEmpty) {
                          if(load) {
                            ...
                            update(titleController.text.toString(), dateController.text.toString());
                          }
