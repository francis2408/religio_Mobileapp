...
class _EditEventScreenState extends State<EditEventScreen> {
  ...
  getEventData() async {
    String eventUrl = '';
    eventUrl = "$baseUrl/search_read/calendar.event?domain=[('id','=',$eventID)]&fields=['name','start_datetime','duration','location','start_date','stop_date','category','calendar_config_type_id','type','allday','description_html','user_id']";
    var request = http.Request('GET', Uri.parse(eventUrl));
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    ...
    if (response.statusCode == 200) {
      ...
      for(int i = 0; i < eventData.length; i++) {
        eventNameController.text = eventData[i]['name'];
        if(eventData[i]['allday'] != 'Yes') {
          _allDay = false;
          allDay = 'False';
          if(eventData[i]['start_datetime'] != '' && eventData[i]['start_datetime'] != null) {
            startAtDateController.text = DateFormat('dd-MM-yyyy hh:mm a').format(DateTime.parse(eventData[i]['start_datetime']));
            startAt = DateFormat('yyyy-MM-dd hh:mm:ss').format(DateTime.parse(eventData[i]['start_datetime']));
            startDate = DateFormat('yyyy-MM-dd').format(DateTime.parse(eventData[i]['start_datetime']));
            startDateController.text = DateFormat('dd-MM-yyyy').format(DateTime.parse(eventData[i]['start_datetime']));
            endDate = DateFormat('yyyy-MM-dd').format(DateTime.parse(eventData[i]['start_datetime']));
            endDateController.text = DateFormat('dd-MM-yyyy').format(DateTime.parse(eventData[i]['start_datetime']));
          }
        } else {
          _allDay = true;
          allDay = 'True';
          startDate = eventData[i]['start_date'];
          startDateController.text = DateFormat('dd-MM-yyyy').format(DateTime.parse(eventData[i]['start_date']));
          endDate = eventData[i]['stop_date'];
          endDateController.text = DateFormat('dd-MM-yyyy').format(DateTime.parse(eventData[i]['stop_date']));
          startAt = eventData[i]['start_date'] + " 12:00:00";
        }
        type = eventData[i]['type'];
        if(eventData[i]['calendar_config_type_id'].isNotEmpty && eventData[i]['calendar_config_type_id'] != []) {
          calendarID = eventData[i]['calendar_config_type_id'][0].toString();
          calendar = eventData[i]['calendar_config_type_id'][1];
        }
        placeController.text = eventData[i]['location'];
        descriptionController.text = eventData[i]['description_html'].replaceAll(exp, '');
      }
    } else {
      var message = json.decode(await response.stream.bytesToString())['message'];
      setState(() {
  ...
  update(String eventName) async {
    String name = eventName;
    String location = placeController.text.toString();
    String description = descriptionController.text.toString();
    var request = http.MultipartRequest('POST',  Uri.parse('$baseUrl/write/calendar.event?ids=[$eventID]'));
    _allDay ? request.fields.addAll({
      'values': "{'name': '$name','allday': $allDay,'start': '$startAt','stop': '$startAt','start_date': '$startDate','stop_date': '$endDate','type': '$type','user_id': $userId,'calendar_config_type_id': ${calendarID != '' ? calendarID : []},'location': '$location','description_html': '$description','category': 'calendar'}"
    }) : request.fields.addAll({
      'values': "{'name': '$name','allday': $allDay,'start': '$startAt','stop': '$startAt','type': '$type','user_id': $userId,'calendar_config_type_id': ${calendarID != '' ? calendarID : []},'location': '$location','description_html': '$description','category': 'calendar'}"
    });
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    if(response.statusCode == 200) {
                  ...
                  Container(
                    ...
                    child: DropDownTextField(
                      ...
                      textFieldDecoration: InputDecoration(
                        ...
                        hintText: calendar != '' ? calendar : "Select calendar type",
                        hintStyle: GoogleFonts.breeSerif(
                          color: calendar != '' ? valueColor : labelColor2,
                          fontStyle: calendar != '' ? FontStyle.normal : FontStyle.italic,
                        ),
                        enabledBorder: OutlineInputBorder(
                          borderRadius: BorderRadius.circular(10),
                          borderSide: const BorderSide(
                      ...
                      if(_allDay) {
                        if(eventNameController.text.isNotEmpty && startDateController.text.isNotEmpty && endDateController.text.isNotEmpty) {
                          if(load) {
                            ...
                            update(eventNameController.text.toString());
                          }
                        } else {
                          setState(() {
