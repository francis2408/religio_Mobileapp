...
class NoResult extends StatelessWidget {
  const NoResult({Key? key, required this.onPressed, required this.text,}) : super(key: key);
...
class _ConditionalFloatingActionButtonState extends State<ConditionalFloatingActionButton> {
  Timer? glowTimer;
  ...
  Widget build(BuildContext context) {
    return widget.isEmpty ? AnimatedContainer(
      ...
      decoration: BoxDecoration(
        ...
        color: widget.iconBackColor.withOpacity(glowOpacity),
        boxShadow: [
          BoxShadow(
            color: widget.iconBackColor.withOpacity(glowOpacity),
            blurRadius: 10.0,
...
class CustomProfileBottomSheet extends StatelessWidget {
  ...
  const CustomProfileBottomSheet({Key? key, required this.size, required this.onGalleryPressed, required this.onCameraPressed, required this.onRemovePressed,}) : super(key: key);
...
class CustomTabBar extends StatefulWidget {
  const CustomTabBar({Key? key, required this.tabController, required this.tabs, required this.onTabTap,}) : super(key: key);
