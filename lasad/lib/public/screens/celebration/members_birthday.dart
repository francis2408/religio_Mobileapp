...
class MembersBirthdayScreen extends StatefulWidget {
  const MembersBirthdayScreen({Key? key}) : super(key: key);

  @override
  State<MembersBirthdayScreen> createState() => _MembersBirthdayScreenState();
}

class _MembersBirthdayScreenState extends State<MembersBirthdayScreen> {
  ...
  getBirthdayData() async {
    ...
    if(sectorTab == 'Indian Sector' && celebrationName == 'Birthday') {
      request = http.Request('GET', Uri.parse("$baseUrl/province/member/birthday/$userProvinceId"));
    } else {
      request = http.Request('GET', Uri.parse("$baseUrl/province/member/birthday/$sri_sector_id"));
    }
    ...
    if (response.statusCode == 200) {
      ...
      thisMonthBirthday = result['data'];
      upcomingBirthday = result['next_30days'];
    } else {
  ...
  getFeastData() async {
    ...
    if(sectorTab == 'Indian Sector' && celebrationName == 'Feast') {
      request = http.Request('GET', Uri.parse("$baseUrl/province/member/feast/$userProvinceId"));
    } else {
      request = http.Request('GET', Uri.parse("$baseUrl/province/member/feast/$sri_sector_id"));
    }
    http.StreamedResponse response = await request.send();
    if (response.statusCode == 200) {
      var result = json.decode(await response.stream.bytesToString());
      ...
      thisMonthFeast = result['data'];
      upcomingFeast = result['next_30'];
    } else {
      var message = json.decode(await response.stream.bytesToString())['message'];
      setState(() {
        _thisMonth = false;
