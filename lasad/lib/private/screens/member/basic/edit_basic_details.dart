...
class _EditBasicDetailsScreenState extends State<EditBasicDetailsScreen> {
  ...
  FocusNode placeBirthFocusNode = FocusNode();
  ...
  getMemberBasicData() async {
    var request =  userMember == 'Member' ? http.Request('GET', Uri.parse("$baseUrl/search_read/res.member?domain=[('id','=',$id)]&fields=['name','middle_name','image_1920','last_name','member_type','title','place_of_birth','is_dob_or_age','dob','age','street','street2','place','city','district_id','state_id','country_id','zip','mobile','email','blood_group_id','mother_tongue_id','known_language_ids','aadhar_proof','pan_proof','voter_proof','passport_proof','aadhar_proof_name','voter_proof_name','pan_proof_name','passport_proof_name','unique_code','voter_id','pancard_no','passport_no']")) : http.Request('GET', Uri.parse("$baseUrl/search_read/res.member?domain=[('id','=',$memberId)]&fields=['name','middle_name','image_1920','last_name','member_type','title','place_of_birth','is_dob_or_age','dob','age','street','street2','place','city','district_id','state_id','country_id','zip','mobile','email','blood_group_id','mother_tongue_id','known_language_ids','aadhar_proof','pan_proof','voter_proof','passport_proof','aadhar_proof_name','voter_proof_name','pan_proof_name','passport_proof_name','unique_code','voter_id','pancard_no','passport_no']"));
    ...
    if (response.statusCode == 200) {
      ...
      for(int i = 0; i < memberBasic.length; i++) {
        ...
        placeController.text = memberBasic[i]['place'];
  ...
  update(String name,memberType) async {
    if(firstNameController.text.isNotEmpty && mType.isNotEmpty) {
      ...
      String placeBirth = placeOfBirthController.text.toString();
      ...
      Map<String, dynamic> fields = {
        'name':'$name','middle_name':'$middleName','last_name':'$lastName','gender':'$gender','is_dob_or_age':'$isDOBandAge','dob':'$dateOfBirth','age': age,'member_type':'$memberType','place_of_birth':'$placeBirth','known_language_ids': knownLanguage,'mobile':'$mobile','email':'$email','street':'$street','street2':'$street2','place': '$place','city':'$city','zip': '$zip','unique_code': '$aadharNo','voter_id': '$voterNo','pancard_no': '$panNo','passport_no': '$passNo'
      };
                        ...
                        child: Column(
                          children: [
                            ...
                            RadioListTile(
                              ...
                              value: 'Lay Brother',
                              groupValue: mType,
                              title: Text('Lay Brother', style: GoogleFonts.breeSerif(color: Colors.black87, fontSize: size.height * 0.018,),),
                            ...
                            if(isDob == true) Container(
                              ...
                              child: TextFormField(
                                ...
                                onFieldSubmitted: (val) {
                                  ...
                                  FocusScope.of(context)
                                      .requestFocus(placeBirthFocusNode);
                                },
                              ),
                            ),
