...
import 'other_institution_details.dart';
import 'other_institution_members_list.dart';

class OtherInstitutionListScreen extends StatefulWidget {
  const OtherInstitutionListScreen({Key? key}) : super(key: key);

  @override
  State<OtherInstitutionListScreen> createState() => _OtherInstitutionListScreenState();
}

class _OtherInstitutionListScreenState extends State<OtherInstitutionListScreen> {
  ...
  void _loadMore() async {
    if (_hasNextPage == true &&
      ...
      page += limit;
      var request = http.Request('GET', Uri.parse("""$baseUrl/search_read/res.institution?fields=['name','image_512','superior_id','ministry_category_id','members_count']&domain=[('id','!=',$userInstituteId),('rel_province_id','=',$userProvinceId),('is_abroad','=',False),('is_other_diocese','=',False)]&context={"bypass":1}&limit=$limit&offset=$page&order=name asc"""));
  ...
  Future<void> getInstitutionListData(String searchWord) async {
    ...
    var request = http.Request('GET', Uri.parse("""$baseUrl/search_read/res.institution?fields=['name','image_512','superior_id','ministry_category_id','members_count']&domain=[('name','ilike','$searchName'),('id','!=',$userInstituteId),('rel_province_id','=',$userProvinceId),('is_abroad','=',False),('is_other_diocese','=',False)]&context={"bypass":1}&limit=500&offset=0&order=name asc"""));
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
              ...
              children: [
                ...
                Row(
                  ...
                  children: [
                    ...
                    Text(institutionCount, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.bold, color: countValue),)
                  ],
                ),
                SizedBox(
                  height: size.height * 0.01,
                                            ...
                                            child: Container(
                                              ...
                                              child: Column(
                                                ...
                                                children: [
                                                  ...
                                                  institutionData[index]['superior_id'].isNotEmpty && institutionData[index]['superior_name'] != [] ? SizedBox(
                                                    height: size.height * 0.01,
                                                  ) : Container(),
                                                  Row(
                                                    children: [
