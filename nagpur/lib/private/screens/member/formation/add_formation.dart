  ...
  save(String stage, startYear, state) async {
    if(stage.isNotEmpty && stage != '' && startYear != null && startYear != '') {
      ...
      userMember == 'Member' ? request.fields.addAll({
        'values': "{'formation_stage_id': $stageID,'member_id': $id,'house_id': ${houseID != '' && houseID != null ? houseID : []},'start_year': '$startYear','end_year': '$endYear','study_info': '$studyInfo','state': '$state'}"
      }) : request.fields.addAll({
        'values': "{'formation_stage_id': $stageID,'member_id': $memberId,'house_id': ${houseID != '' && houseID != null ? houseID : []},'start_year': '$startYear','end_year': '$endYear','study_info': '$studyInfo','state': '$state'}"
      });
      request.headers.addAll(headers);
      http.StreamedResponse response = await request.send();
      if(response.statusCode == 200) {
