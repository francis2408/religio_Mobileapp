...
import 'package:scb/private/screens/event/add_event.dart';
...
class _AllEventScreenState extends State<AllEventScreen> {
  ...
  void _loadMore() async {
    if (_hasNextPage == true &&
      ...
      if(selectedTab == 'All') {
        request = http.Request('GET', Uri.parse("""$baseUrl/search_read/calendar.event?fields=['name','start_datetime','duration','location','start_date','stop_date','category','calendar_config_type_id','allday','description_html','user_id','recurrency','community_id']&domain=[('type','=','public'),('category','=','calendar')]&limit=$eventLimit&offset=$eventPage&order=stop_date desc"""));
        ...
        request = http.Request('GET', Uri.parse("""$baseUrl/search_read/calendar.event?fields=['name','start_datetime','duration','location','start_date','stop_date','category','calendar_config_type_id','allday','description_html','user_id','recurrency','community_id']&domain=[('type','=','public'),('category','=','calendar'),('calendar_config_type_id.name','=','Community Calendar')]&limit=$eventLimit&offset=$eventPage&order=stop_date desc"""));
      } else {
        request = http.Request('GET', Uri.parse("""$baseUrl/search_read/calendar.event?fields=['name','start_datetime','duration','location','start_date','stop_date','category','calendar_config_type_id','allday','description_html','user_id','recurrency','community_id']&domain=[('category','=','calendar'),('user_id','=',$userId)]&limit=$eventLimit&offset=$eventPage&order=stop_date desc"""));
      }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
                        ...
                        itemBuilder: (BuildContext context, int index) {
                          ...
                          return eventData[index]['allday'] != 'Yes' ? GestureDetector(
                            onTap: () {
                              ...
                              } else if (selectedTab == 'Community' && userRole == 'House/Community') {
                                if(userId == deleteID) {
                                  Scaffold.of(context).showBottomSheet<void>((BuildContext context) {
                                  return CustomBottomSheet(
                                    size: size, // Pass the 'size' variable
                                  ...
                                  Container(
                                    ...
                                    child: Column(
                                      ...
                                      children: [
                                        ...
                                        if(userCommunityId != eventData[index]['community_id'][0]) SizedBox(height: size.height * 0.01,),
                                        if(userCommunityId != eventData[index]['community_id'][0]) Row(
                                          children: [
                                            Container(width: size.width * 0.08, alignment: Alignment.topLeft, child: const Icon(Icons.other_houses, color: menuPrimaryColor,),),
                                            eventData[index]['community_id'].isNotEmpty && eventData[index]['community_id'] != [] ? Text(eventData[index]['community_id'][1], style: GoogleFonts.signika(color: Colors.black87, fontSize: size.height * 0.021),) : Text("-", style: GoogleFonts.secularOne(fontSize: size.height * 0.02, color: Colors.grey, fontStyle: FontStyle.italic),),
                                          ],
                                        ),
                                        SizedBox(height: size.height * 0.01,),
                                        Row(
      ...
      floatingActionButton: selectedTab == 'My Calendar' ? eventData.isEmpty ? ConditionalFloatingActionButton(
        ...
        child: const Icon(Icons.add, color: buttonIconColor,),
      ) : selectedTab == 'Community' && userRole != 'Member' ? eventData.isEmpty ? ConditionalFloatingActionButton(
        isEmpty: true,
        iconBackColor: iconBackColor,
        onPressed: () async {
          String refresh = await Navigator.push(context,
