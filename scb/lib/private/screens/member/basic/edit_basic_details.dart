...
class _EditBasicDetailsScreenState extends State<EditBasicDetailsScreen> {
  ...
  List memberType = [
    {"id": 1, "name": "Working Sister"},
    {"id": 2, "name": "Junior Sister"},
    {"id": 3, "name": "Novice"},
    {"id": 4, "name": "Candidacy"},
    {"id": 5, "name": "Postulancy"},
    {"id": 6, "name": "Retired Sister"},
  ];
  ...
  getMemberTypeData() {
    List data = memberType;
    for(int i = 0; i < data.length; i++) {
      memberTypeDropDown.add(DropDownValueModel(name: data[i]['name'], value: data[i]['id']));
    }
  ...
  getTitleBased(type) async {
    var request;
    if(type == 'Sister') {
      request = http.Request('GET', Uri.parse("$baseUrl/search_read/res.partner.title?domain=[('for_sister','=',True)]&fields=['name']"));
    } else if(type == 'Junior Sister') {
      request = http.Request('GET', Uri.parse("$baseUrl/search_read/res.partner.title?domain=[('for_sister','=',True)]&fields=['name']"));
    } else if(type == 'Novice') {
      ...
      request = http.Request('GET', Uri.parse("$baseUrl/search_read/res.partner.title?domain=[('for_sister','=',True)]&fields=['name']"));
    } else if(type == 'Postulancy') {
      request = http.Request('GET', Uri.parse("$baseUrl/search_read/res.partner.title?domain=[('for_sister','=',True)]&fields=['name']"));
    } else if(type == 'Retired Sister') {
      request = http.Request('GET', Uri.parse("$baseUrl/search_read/res.partner.title?domain=[('for_sister','=',True)]&fields=['name']"));
    } else {
      request = http.Request('GET', Uri.parse("$baseUrl/search_read/res.partner.title?fields=['name']"));
    }
    request.headers.addAll(headers);
    http.StreamedResponse response = await request.send();
    if(response.statusCode == 200) {
  ...
  update(String name,memberType) async {
    if(firstNameController.text.isNotEmpty && mType.isNotEmpty) {
      ...
      Map<String, dynamic> fields = {
        'name':'$name','middle_name':'$middleName','last_name':'$lastName','gender':'$gender','is_dob_or_age':'$isDOBandAge','dob':'$dateOfBirth','age': age,'member_type':'$memberTypes','place_of_birth':'$place','known_language_ids': knownLanguage,'mobile':'$mobile','email':'$email','street':'$street','street2':'$street2','city':'$city','zip': '$zip','unique_code': '$aadharNo','voter_id': '$voterNo','pancard_no': '$panNo','passport_no': '$passNo'
      };
                            ...
                            Container(
                              ...
                              child: DropDownTextField(
                                ...
                                textFieldDecoration: InputDecoration(
                                  ...
                                  hintStyle: GoogleFonts.breeSerif(
                                    color: mType != '' ? Colors.black87 : labelColor2,
                                    fontStyle: mType != '' ? FontStyle.normal : FontStyle.italic,
                                  ),
                                  enabledBorder: OutlineInputBorder(
                                    borderRadius: BorderRadius.circular(10),
                                    borderSide: const BorderSide(
